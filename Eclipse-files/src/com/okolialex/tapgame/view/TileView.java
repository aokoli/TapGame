...
public class TileView extends View {
    ...
    public void onDraw(Canvas canvas) {
        ...
        for (int x = 0; x < mXTileCount; x += 1) {
            for (int y = 0; y < mYTileCount; y += 1) {
                if (mTileGrid[x][y] > 0) {
                    canvas.drawBitmap(mTileArray[mTileGrid[x][y]], mXOffset + x * mTileSize, // TODO Offset seems to be a 1-time occurence. Why?
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        mXTileCount = (int) Math.floor(w / mTileSize);  // TODO why floor?
        mYTileCount = (int) Math.floor(h / mTileSize);

        mXOffset = ((w - (mTileSize * mXTileCount)) / 2);  // TODO Why isn't divided by num of tiles? Why 2?
        mYOffset = ((h - (mTileSize * mYTileCount)) / 2);

        mTileGrid = new int[mXTileCount][mYTileCount];
        setTouchedRanges(); // TODO Alex added.
        clearTiles();
    }
